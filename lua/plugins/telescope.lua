return {
  ...
  config = function()
    ...
    map("n", "<leader>ff", builtin.find_files, { desc = "Lists files in current working directory" }, opts) -- Lists files in your current working directory, respects .gitignore
    map("n", "<leader>fx", builtin.treesitter, { desc = "Lists tree-sitter symbols" }, opts)                -- Lists tree-sitter symbols
    -- 单词拼写
    -- map("n", "<leader>fs", builtin.spell_suggest, { desc = "Lists spell options" }, opts)                   -- Lists spell options
  end
}
