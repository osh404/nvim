...
-- 根据是否为 VS Code 动态设置插件 spec
local is_vscode = vim.g.vscode == 1
local spec
if is_vscode then
    spec = {
        -- ========== VS Code 模式：不加载 LazyVim 官方插件，只加载你的自定义插件 ==========
        -- 只导入你需要的插件，不要导入 lazyvim.plugins
        { import = "plugins/vscode" }, -- 和下面的比较是两种写法, 或者统一加上：vscode=true （lazyvim和vscode一起使用同一个配置）
        -- { import = "plugins/flash" },
    }
else
    spec = {
        -- ========== 原生 Neovim 模式：加载完整 LazyVim ==========
        { "LazyVim/LazyVim",            import = "lazyvim.plugins" },
        -- 你的自定义插件
        { import = "plugins/alpha" },      -- 首页
        { import = "plugins/dracula" },    -- 德古拉主题
        { import = "plugins/trouble" },    -- 错误提示
        { import = "plugins/nvim-cmp" },   -- 自动补全
        { import = "plugins/telescope" },  -- 搜索
        { import = "plugins/mason" },      -- 语言管理
        { import = "plugins/lsp" },        -- LSP客户端
        { import = "plugins/go-debug" },   -- debug
        { import = "plugins/treesitter" }, -- 语法高亮
        { import = "plugins/lualine" },    -- 底部状态栏
        { import = "plugins/go-vim" },     --  go语言开发
        { import = "plugins/im-select" },  -- 中英转换
        { import = "plugins/gitsigns" },   -- git 状态标记
        { import = "plugins/autopairs" },  -- 括号自动配对（输入大括号自动换行缩进）
        { import = "plugins/neo-tree" },   -- 目录树
        -- { import = "plugins/claude-code" }, -- ai claude-code
    }
end

require("lazy").setup({
    spec = spec, -- 使用动态生成的 spec
    --   spec = {
